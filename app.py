...
PINK="#E5007D"; LIGHT="#FFF0F7"; DARK="#2B1B25"
st.markdown(f"""
<style>
...
for target,opts in aliases.items():
    ...
    mapping[target]=st.sidebar.selectbox(target,["—"]+list(tx.columns),index=(list(tx.columns).index(guess)+1 if guess in tx.columns else 0))
...
for c in ["margin","discount","points_earned","points_redeemed","promotion_used"]:
    if c in tx: tx[c]=pd.to_numeric(tx[c],errors="coerce").fillna(0)
tx=tx.dropna(subset=["customer_id","date"])

as_of=tx.date.max()+pd.Timedelta(days=1)
g=tx.groupby("customer_id").agg(last_purchase=("date","max"),frequency=("customer_id","count"),monetary=("net_sales","sum")).reset_index()
g["recency_days"]=(as_of-g.last_purchase).dt.days
g["avg_basket"]=g.monetary/g.frequency

if "margin" in tx:
    m=tx.groupby("customer_id").margin.sum()
    g["margin"]=g.customer_id.map(m).fillna(0); g["margin_rate"]=g.margin/g.monetary.replace(0,np.nan)
else:g["margin"]=np.nan;g["margin_rate"]=np.nan
...
g["opportunity_score"]=g.R_score*0.45+g.promo_share.rank(pct=True)*5*0.35+g.category_breadth.rank(pct=True)*5*0.20
v=g.value_score>=g.value_score.median();o=g.opportunity_score>=g.opportunity_score.median()
g["segment"]=np.select([v&o,v&~o,~v&o],["High Value / High Opportunity","High Value / Low Opportunity","Low Value / High Opportunity"],default="Low Value / Low Opportunity")
g["strategy"]=g.segment.map({"High Value / High Opportunity":"Protect & Grow","High Value / Low Opportunity":"Retain Efficiently","Low Value / High Opportunity":"Develop","Low Value / Low Opportunity":"Automate / Low-cost"})
...
with t1:
    ...
    seg=g.groupby(["segment","strategy"],as_index=False).agg(customers=("customer_id","count"),revenue=("monetary","sum"),avg_basket=("avg_basket","mean"),frequency=("frequency","mean"))
...
with t2:
    st.subheader("RFM")
    st.dataframe(g[["customer_id","recency_days","frequency","monetary","avg_basket","R_score","F_score","M_score","segment"]].head(500),use_container_width=True,hide_index=True)
    st.caption("First 500 rows shown.")

with t3:
    st.subheader("Loyalty economics")
    if "points_earned" in tx:
        earned=tx.points_earned.sum(); redeemed=tx.points_redeemed.sum() if "points_redeemed" in tx else 0
        a,b,c=st.columns(3);a.metric("Points earned",f"{earned:,.0f}");b.metric("Points redeemed",f"{redeemed:,.0f}");c.metric("Reward cost @ €0.01",f"€{redeemed*.01:,.0f}")
    ...
    else: st.warning("Points data not provided.")
    st.markdown("### Next step")
    st.write("Compare current rewards with targeted treatments using incremental revenue, incremental margin, retention and loyalty cost.")
...
st.caption("Synthetic data — created to demonstrate analytical approach. Results are not BIPA actuals or forecasts.")
